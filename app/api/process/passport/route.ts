...
import { processPassportPhoto, validateImageBuffer } from "@/lib/sharp-utils";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { imageId, country } = parsed.data;
    ...
    await supabaseAdmin
      .from("images")
      .update({ processed_url: upload.secureUrl, cloudinary_processed_id: upload.publicId, status: "completed", metadata: { country } })
      .eq("id", imageId);
    ...
    return NextResponse.json({
      success: true,
      data: { imageId, originalUrl: img.original_url, processedUrl: upload.secureUrl, toolUsed: "passport_photo", country },
    });
  } catch (err) {
    console.error("[process/passport]", err);
