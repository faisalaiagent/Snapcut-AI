...
export function WhiteBgClient() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/process/white-bg-public", { method: "POST", body: formData });
  ...
  const handleReset = () => { setPreviewUrl(null); setResultUrl(null); setError(null); setProcessing(false); };
  ...
  const handleDownload = async () => {
    ...
    const a = document.createElement("a"); a.href = url; a.download = "snapcut-white-bg.jpg"; a.click();
  ...
  if (processing) return (
    <div className="card p-12 text-center">
      ...
      <p className="text-[15px] font-medium text-ink mb-1">Generating white background…</p>
      <p className="text-[13px] text-ink-4">Removing original background and applying white</p>
    </div>
  );
  ...
  if (resultUrl && previewUrl) return (
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        <div className="rounded-2xl overflow-hidden bg-surface-2 aspect-square flex items-center justify-center relative">
        ...
        <div className="rounded-2xl overflow-hidden bg-white border border-surface-3 aspect-square flex items-center justify-center relative">
      ...
      <div className="flex gap-3">
        ...
        <button onClick={handleReset} className="btn-secondary px-4 gap-2"><RefreshCw className="w-4 h-4" />New Image</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-[17px] font-medium text-ink mb-2">Upload your product photo</p>
        <p className="text-[14px] text-ink-4 mb-5">We&apos;ll remove the background and add pure white automatically</p>
        <div className="flex justify-center gap-2 flex-wrap">
          {["JPG", "PNG", "WEBP", "Free", "No signup"].map((f) => (
