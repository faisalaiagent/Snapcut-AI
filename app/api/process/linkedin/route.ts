...
import { optimiseLinkedIn, validateImageBuffer } from "@/lib/sharp-utils";
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: { imageId, originalUrl: img.original_url, processedUrl: upload.secureUrl, toolUsed: "linkedin_photo" },
    });
  } catch (err) {
    console.error("[process/linkedin]", err);
