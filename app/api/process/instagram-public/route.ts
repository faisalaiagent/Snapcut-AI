...
import { optimiseInstagramDp } from "@/lib/sharp-utils";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const upload = await uploadBuffer(resultBuffer, {
      ...
      tags: ["instagram", "public"],
    });

    return NextResponse.json({
      success: true,
      data: { processedUrl: upload.secureUrl, toolUsed: "instagram_dp" },
    });
  } catch (err) {
    console.error("[instagram-public]", err);
    return NextResponse.json(
