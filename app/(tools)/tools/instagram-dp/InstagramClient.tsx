...
export function InstagramClient() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/process/instagram-public", { method: "POST", body: formData });
  ...
  const handleDownload = async () => {
    ...
    a.href = url; a.download = "instagram-dp.jpg"; a.click();
  ...
  if (processing) return (
    <div className="card p-12 text-center">
      ...
      <p className="text-[15px] font-medium text-ink">Optimizing for Instagram…</p>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-5">
        <div className="text-center">
          <p className="text-[12px] text-ink-4 mb-2">Feed</p>
          <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-[#e1306c]/30 bg-surface-2">
            <img src={resultUrl} alt="Instagram feed preview" className="w-full h-full object-cover" />
          </div>
        </div>
        <div className="text-center">
          <p className="text-[12px] text-ink-4 mb-2">Stories</p>
          <div className="w-16 h-16 rounded-full overflow-hidden border-[3px] border-[#e1306c] bg-surface-2">
            <img src={resultUrl} alt="Instagram stories preview" className="w-full h-full object-cover" />
          </div>
        </div>
        <div className="flex-1 bg-accent-light border border-[#b3dcc9] rounded-xl px-4 py-3 text-[14px] text-accent-2">
          ✓ Optimized to 320×320px — ready to upload to Instagram
        </div>
      </div>

      <div className="grid sm:grid-cols-2 gap-4">
        {previewUrl && (
          <div className="rounded-2xl bg-surface-2 aspect-square flex items-center justify-center overflow-hidden relative">
    ...
    <div className="space-y-4">
      <label
        className={cn(
          ...
          dragging && "bg-accent-light border-accent"
        )}
      ...
      >
        ...
        <p className="text-[17px] font-medium text-ink mb-2">Upload your photo</p>
        <p className="text-[14px] text-ink-4">Square or portrait photos work best · No signup needed</p>
      </label>
