...
import { optimiseLinkedIn } from "@/lib/sharp-utils";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const upload = await uploadBuffer(resultBuffer, {
      ...
      tags: ["linkedin", "public"],
    });

    return NextResponse.json({
      success: true,
      data: { processedUrl: upload.secureUrl, toolUsed: "linkedin_photo" },
    });
  } catch (err) {
    console.error("[linkedin-public]", err);
    return NextResponse.json(
